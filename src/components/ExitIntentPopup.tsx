...
export default function ExitIntentPopup() {
  ...
  useEffect(() => {
    ...
    // Test trigger - show popup after 3 seconds for demo
    const demoTimer = setTimeout(() => {
      if (!hasShown && !popupShown) {
        console.log('Demo: Exit popup triggered after 3 seconds')
      }
    }, 3000)
    ...
    // Also trigger on scroll to top aggressively
    const handleScroll = () => {
      if (window.scrollY === 0 && !hasShown && !popupShown) {
        const randomChance = Math.random()
        if (randomChance > 0.7) { // 30% chance when at top
          setIsVisible(true)
          setHasShown(true)
          sessionStorage.setItem('exitPopupShown', 'true')
        }
